...
def show_image(im):
    ...
    x = plt.figure(1)
    plt.clf()
    plt.imshow(im, vmin = da.min(im), vmax = da.max(im))
    plt.title('im')
    plt.show(block=False)
    while(plt.fignum_exists(1)):
        try:
...
def search(graph):
    ...
    for key, value in graph.iteritems():
        if 'array' in key[0].split('-')[0]:
            if len(value) == 2048:
                dict_parc = {}
                dict_parc['value'] = value 
                dict_parc['key'] = key
                dict_fin.append(dict_parc)
    return dict_fin
                 
main()


sky_graph = collections_to_dsk(sky)
dirty_graph = collections_to_dsk(dirty)
fr_graph = collections_to_dsk(fr)
quad_graph = collections_to_dsk(quad)


dict_sky = search(sky_graph)
dict_dirty = search(dirty_graph)
dict_fr = search(fr_graph)
dict_quad = search(quad_graph)
...
for x in dict_quad:	
    if compare(x['value'], dict_sky[0]['value']):
        print dict_sky[0]['key'][0]
    elif compare(x['value'], dict_dirty[0]['value']):    
        print dict_dirty[0]['key'][0]
    elif compare(x['value'], dict_fr[0]['value']):    
        print dict_fr[0]['key'][0]
    else:
        print('FALSE')

for key, value in quad_graph.iteritems():
    print value    
'''
    if 'array' in key[0].split('-')[0] or :
        print key 
    '''
...
print(key_fr)
print(value_fr)
print(key_quad == key_sky)
print(value_quad == value_sky)

print(key_quad == key_dirty)
print(value_quad == value_dirty)

print(key_quad == key_fr)
print(value_quad == value_fr)
