...
def calcul(images):
    #images = [dirty, psf, sky]
    #Parametres du code
    lamb = 0.0005
    
    fr_npy = ir2fr(images[1], images[2].shape)
    fr = da.from_array(fr_npy, chunks = images[1].shape)
    quad = wiener(images[0], da.zeros_like(images[2]), fr, fr_npy, lamb)
